            ...
            <div class="section-body">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>Edit {{ $backUrl }} About {{ $project->title }}</h4>
                    </div>
                    <div class="card-body">

                        <form method="post" action="{{ route($backUrl . '.update', $project->slug) }}"
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group mb-4">
                                                ...
                                                <input type="date" class="form-control" name="completed_at"
                                                    value="{{ $project->completed_at->format('Y-m-d') }}">
                                                @error('completed_at')
                                                    <span class=" text-danger">{{ $message }}</span>
                                                @enderror
                                            </div>
                                                ...
                                                <select class="form-control select2" name="partner_id">
                                                    @foreach ($partners as $partner)
                                                        <option value="{{ $partner->id }}"
                                                            {{ $partner->id == $project->partner_id ? 'selected' : '' }}>
                                                            {{ $partner->name }}
                                                        </option>
                                                    @endforeach
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-12">
                                            <img id="preview-image-before-upload"
                                                src="{{ asset('images/Project/cover/' . $project->cover) }}"
